...
from Methods.models import AE_CelebA
import torch
...
if __name__ == '__main__':
    ...
    for modelname in ['gmvae', 'rae']:
        if modelname == 'gmvae':
            args.resultpath = 'Results/gmvae_clusters'
            ...
            model.load_state_dict(torch.load('Results/gmvae/model_CelebA.pt'), strict=False)
            prior.load_state_dict(torch.load('Results/gmvae/prior_CelebA.pt'), strict=False)
        elif modelname == 'rae':
            args.resultpath = 'Results/rae_clusters'
            args.model_type = 'deterministic'
            prior = rae.Prior(data_size=[10, args.z_dim])
            model = AE_CelebA(z_dim=args.z_dim, nc=args.nc, model_type=args.model_type)
            model.load_state_dict(torch.load('Results/rae/model_CelebA.pt'), strict=False)
            prior.load_state_dict(torch.load('Results/rae/prior_CelebA.pt'), strict=False)
        elif modelname == 'rvae':
            args.resultpath = 'Results/rvae_clusters'
            args.model_type = 'probabilistic'
            prior = rvae.Prior(data_size=[10, args.z_dim])
            model = AE_CelebA(z_dim=args.z_dim, nc=args.nc, model_type=args.model_type)
            model.load_state_dict(torch.load('Results/rvae/model_CelebA.pt'), strict=False)
            prior.load_state_dict(torch.load('Results/rvae/prior_CelebA.pt'), strict=False)
        else:
            args.resultpath = 'Results/vampprior_clusters'
            args.model_type = 'probabilistic'
            prior = vampprior.Prior(data_size=[10, args.nc, 28, 28], device=device)
            model = AE_CelebA(z_dim=args.z_dim, nc=args.nc, model_type=args.model_type)
            model.load_state_dict(torch.load('Results/vampprior/model_CelebA.pt'), strict=False)
            prior.load_state_dict(torch.load('Results/vampprior/prior_CelebA.pt'), strict=False)
