...
parser = argparse.ArgumentParser(description='Variational Autoencoder Example')
...
if __name__ == '__main__':
    ...
    loss = vae.train_model(model, src_loaders['train'], src_loaders['val'], device, args)
    ...
    model = model.to('cpu')
    torch.save(model.state_dict(), '{}/vae_model_{}_{}.pt'.format(args.resultpath, args.model_type, args.source_data))
    with open('{}/vae_loss_{}_{}.pkl'.format(args.resultpath, args.model_type, args.source_data), 'wb') as f:
        pickle.dump(loss, f)
    print('\n')
