...
if __name__ == '__main__':
    ...
    # t-sne visualization
    if args.source_data == 'MNIST':
        evaluation.visualization_tsne(model, src_loaders['val'], device, args, prefix='wae')
    else:
        evaluation.visualization_tsne2(model, src_loaders['val'], device, args, prefix='wae')

    # save models and learning results
    model = model.to('cpu')
    torch.save(model.state_dict(),
               '{}/wae_model_{}_{}.pt'.format(args.resultpath, args.model_type, args.source_data))
    with open('{}/wae_loss_{}_{}.pkl'.format(args.resultpath, args.model_type, args.source_data), 'wb') as f:
        pickle.dump(loss, f)
    print('\n')
